...
Massachusetts state law protects vernal pools from development. However, this protection is contingent on a pool being certified to function as critical wildlife habitat and unfortunately the state government does not have an active program to review new pools for potential certification. Instead, the state relies on private citizens requesting pool certification at their discretion. Because submitting a certification request is time consuming work that requires visiting a potential pool to collect biological data, many legitimate pools go uncertified and unprotected. To help citizen groups more effectively protect vernal pools, this paper uses a MassGIS data set of potential vernal pools to identify geographic "hot spots" of pools that are more likely to benefit from a certification request.
...
Because of their signifcance as critical habitat, vernal pools are protected under a variety of Massachusetts state and local laws, the most significant of which is the Wetlands Protection Act (WPA). Under the WPA, development that may impact the function of vernal pools as wildlife habitat is not permitted within 100 feet of the boundary of a vernal pool (MA DFW, 2009). But, to qualify for WPA protection, a pools must meet two criteria. First, it must exist within 100 feet of a wetland “Resource Area” that falls under WPA jurisdiction (MA DFW, 2009). Second, it must be certified by the Natural Heritage and Endangered Species Program (NHESP) as functioning biologically as a vernal pool (MA DFW, 2009).

Fortunately, WPA jurisdiction is quite broad, encompassing essentially all wetlands in Massachusetts (310 CMR 10.02 § 1, 1987). But, the requirement that vernal pools be certified does significantly constrain their protection. The reason for this is that certification is a passive process conducted on a volunteer basis by private citizens, not an active process by a government body (MA DFW, 2009). If a citizen would like to request certification of a potential vernal pool, they must visit the pool, collect biological demonstrating the presence of certain species, and submit them to the NHESP for review.  Only once approved as a certified vernal pool and only once it has been established that the pool falls under WPA jurisdiction is it protected from development.

To aid the protection of vernal pools, the NHESP developed a data set of potential vernal pool GIS coordinates based on aerial photography that took place between 1993 and 2000 (Burne, 2001). This data set is a useful starting point for citizens groups who'd like to review potential vernal pools and submit them for certification. However, the volume of records in this data set (`r prettyNum(nrow(potential.pools), big.mark = ",", scientific = FALSE)` as of January 18, 2024) presents a prioritization problem. Citizen groups may have limited time to visit pools, gather data, and request certification. It's unclear on which pools they should focus their limited resources.

In this analysis, I aim to contribute to vernal pool conservation in Massachusetts by flagging pools in the NHESP potential pool data set that are more likely to benefit from protection, face higher threats from development, and can be reviewed more efficiently based on their geographic clustering.
...
terra::plot(
  ...
  cex = .5)
```
For this analysis, I used `r substring(R.Version()$version.string, 1, 15)`, relying primarily on the terra and sf packages (see [Rmd](https://github.com/Skirnir3141/VernalPoolMiniProject/blob/master/MichaelJordanVernalPoolMiniProject.Rmd) for full code). To make the analysis tractable, I chose to focus on a set of towns in Central Massachusetts that (just so happen) to include my hometown. Data on [municipal boundaries](https://www.mass.gov/info-details/massgis-data-municipalities), [potential vernal pool geo points](https://www.mass.gov/info-details/massgis-data-nhesp-potential-vernal-pools), [wetland boundary vectors](https://www.mass.gov/info-details/massgis-data-massdep-wetlands-2005), and [land cover/use vectors](https://www.mass.gov/info-details/massgis-data-2016-land-coverland-use) was downloaded from the MassGIS website on January 18, 2024. Potential pool geo points are dated to between 1993 and 2000, wetland vectors to 2005, and land cover/use vectors to 2016. Potential pools and wetlands were plotted versus municipal boundaries in the NAD83 / Massachusetts Mainland projection and visually inspected for errors.
...
Because a pool must exist within 100 feet of a wetland to be eligible for WPA protection, I evaluated each pool's proximity to a wetland. Pool points that were within 150 feet of the nearest wetland were deemed to be candidates for certification and the rest were discarded. This 150 foot threshold was semi-arbitrary, based on potential pool geo point precision of +/- 15 feet (Burne, 2001) with an additional 20 foot buffer to be conservative.
...
# Create an object that drops potential pools that are more than 150 feet from a
...
Finally, I computed a prioritization index for each potential pool cluster. This index was an equal weighting between a dense rank of the number of pools in the cluster and a dense rank of the average distance to the nearest impervious feature among pools in the cluster. This index structure is (again) semi-arbitrary, but the idea is that the more pools in a group and the closer the group is to human development the more important it is to visit and review.

## Discussion

This is a fairly quick-and-dirty back-of-the-envelope analytical approach that may suffer from several sources of bias.

Because it combines data from different time periods, some several decades in the past (1993-2000 for potential pools, 2005 for wetlands, 2016 for land cover/use), results may not reflect current conditions. Wetlands and vernal pools may have shifted boundaries, disappeared altogether, or been developed over, confounding my attempts to identify high priority vernal pools.

There is also likely to be bias in the potential pool data itself. Potential pools were recorded based on visual inspection of aerial photography, which is necessarily open to human error. The NHESP was able to validate that the false positve rate was low, less than 3% (Burne, 2001). But, it was unable to validate the false negative rate. It's thought to be quite high because small pools below ~50-60 feet in diameter could not be reliably identified due to the granularity of the aerial photographs (Burne, 2001).

Finally, the prioritization index does not account for whether a potential pool is likely to contain species that would allow it to be certified as functioning as a vernal pool. This is a critical dynamic because it's required for pool certification.

## Results

`r nrow(cm.pp[cm.pp$dtw > 150, ])` potential pools were excluded because they were not in close enough proximity to a wetland to fall under WPA jurisdiction. This allowed me to cut down the data set by `r scales::percent(round(nrow(cm.pp[cm.pp$dtw > 150, ]) / nrow(cm.pp), digits = 2))` and prevent wasted effort on pools that are unlikely to fall under WPA jurisdiction.

Within the remaining `r nrow(cm.pp.f)` pools, several hot spots of geographically clustered pools that were atypically close to human development were identified. An example is plotted below and cluster geo coordinates can be provided to interested citizen groups.

```{r graphs, echo=FALSE, fig.align='center', error=FALSE, warning=FALSE, message=FALSE}
# Plot potential pools
par(mar = c(1, 1, 1, 1))
terra::plot(
  sf::st_geometry(
    st_crop(
      cm.towns,
      c(xmin = 196687.2, ymin = 903000, xmax = 202250.2, ymax = 906000))),
  col = "white")
terra::plot(
  sf::st_geometry(cm.wetlands),
  add = TRUE,
...
Aside from potential sources of bias discussed above, the big limitation to this analysis is that its output is not statistically validated. Ideally I'd like to have data on pools that were previously on the potential pool list, were reviewed, and the outcome of that review. Having such data would let me construct and statistically evaluate pool certification models. Perhaps in the future such data could be compiled in partnership with citizen groups.

Regardless, I think this is a reasonable first crack at helping to sort through an otherwise overwhelming amount of potential vernal pools to review.
